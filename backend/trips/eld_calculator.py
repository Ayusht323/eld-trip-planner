...
Leg order :
  1. Drive current_location → pickup_location   (pickup_miles)
  2. Pickup stop 1 hr on-duty
  3. Drive pickup_location  → dropoff_location  (haul_miles)
  4. Dropoff stop 1 hr on-duty
"""
...
def simulate_trip(
    ...
    """
    total_miles  : full route distance (current → pickup → dropoff)
    pickup_miles : deadhead leg distance (current → pickup)
    """
    events: List[Event] = []
    ...
    pickup_miles = max(0.0, min(float(pickup_miles), total_miles))
    pickup_done  = (pickup_miles <= 0.0)
    ...
    def add(status, duration, label, stop_type="", miles_delta=0.0):
        ...
        ev = Event(status, t, t + duration, miles, miles + miles_delta, label, stop_type)
        events.append(ev)
        t     += duration
        miles += miles_delta
        if status == "driving":
    ...
    while miles < total_miles - 0.01:
        ...
        if not pickup_done and miles >= pickup_miles - 0.01:
            pickup_done = True
            add("on_duty", STOP_DURATION, "Pickup — loading cargo", "pickup")
            drive_since_break = 0.0
            continue
        ...
        if not pickup_done:
            miles_to_pickup = pickup_miles - miles
            time_to_pickup  = miles_to_pickup / AVG_SPEED
            if time_to_pickup < max_drive_time - 0.001:
                max_drive_time = time_to_pickup
...
def build_response(events: List[Event], total_miles: float) -> dict:
    ...
    for d in range(num_days):
        ...
        eld_logs.append({"day": d + 1, "segments": filled, "totals": totals})
    ...
    summary = {
        ...
        "rest_stops":    sum(1 for ev in events if ev.stop_type == "rest"),
        "fuel_stops":    sum(1 for ev in events if ev.stop_type == "fuel"),
    }
        ...
        "events": [
            {
                ...
                "end":         round(e.end, 3),
                "miles_start": round(e.miles_start, 1),
                "miles_end":   round(e.miles_end, 1),
                "label":       e.label,
